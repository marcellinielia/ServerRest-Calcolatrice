 ...
 */
public class GetSensoriHandler implements HttpHandler {
    private final Gson gson = new GsonBuilder().setPrettyPrinting().create();
    
    public GetSensoriHandler() {
    }
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            gestisciGetSensori(exchange);
        } catch (Exception e) {
            inviaErrore(exchange, 500, "Errore interno del server: " + e.getMessage());
        }
    }
    
    private void gestisciGetSensori(HttpExchange exchange) throws IOException {
