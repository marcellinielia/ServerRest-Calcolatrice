 ...
 */
public class GetApiariHandler implements HttpHandler {
    private final Gson gson = new GsonBuilder().setPrettyPrinting().create();
    
    public GetApiariHandler() {
    }
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            gestisciGetApiari(exchange);
        } catch (Exception e) {
            inviaErrore(exchange, 500, "Errore interno del server: " + e.getMessage());
        }
    }
    
    private void gestisciGetApiari(HttpExchange exchange) throws IOException {
