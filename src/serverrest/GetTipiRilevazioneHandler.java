 ...
 */
public class GetTipiRilevazioneHandler implements HttpHandler {
    private final Gson gson = new GsonBuilder().setPrettyPrinting().create();
    
    public GetTipiRilevazioneHandler() {
    }
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            gestisciGetTipiRilevazione(exchange);
        } catch (Exception e) {
            inviaErrore(exchange, 500, "Errore interno del server: " + e.getMessage());
        }
    }
    
    private void gestisciGetTipiRilevazione(HttpExchange exchange) throws IOException {
