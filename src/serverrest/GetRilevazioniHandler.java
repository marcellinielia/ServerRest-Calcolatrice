 ...
 */
public class GetRilevazioniHandler implements HttpHandler {
    private final Gson gson = new GsonBuilder().setPrettyPrinting().create();


    public GetRilevazioniHandler() {
    }
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            gestisciGetRilevazioni(exchange);
        } catch (Exception e) {
            inviaErrore(exchange, 500, "Errore interno del server: " + e.getMessage());
        }
    }
    
    private void gestisciGetRilevazioni(HttpExchange exchange) throws IOException {
